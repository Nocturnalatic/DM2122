...
Mesh* MeshBuilder::GenerateQuad(const std::string& meshName, Color color, float length)
{
	...
	v.pos.Set(0.5f * length, 0.5f * length, 0.f);	v.color = color; v.normal.Set(0, 0, 1);	v.texcoord.Set(1, 1); vertex_buffer_data.push_back(v); //v0
	v.pos.Set(-0.5f * length, 0.5f * length, 0.f);	v.color = color; v.normal.Set(0, 0, 1);	v.texcoord.Set(0, 1); vertex_buffer_data.push_back(v); //v1
	...
	v.pos.Set(0.5f * length, -0.5f * length, 0.f);	v.color = color; v.normal.Set(0, 0, 1);	v.texcoord.Set(1, 0); vertex_buffer_data.push_back(v); //v3
...
Mesh* MeshBuilder::GenerateQuadLargeTexture(const std::string& meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;

	v.pos.Set(0.5f * length, 0.5f * length, 0.f);	v.color = color; v.normal.Set(0, 0, 1);	v.texcoord.Set(500, 500); vertex_buffer_data.push_back(v); //v0
	v.pos.Set(-0.5f * length, 0.5f * length, 0.f);	v.color = color; v.normal.Set(0, 0, 1);	v.texcoord.Set(0, 500); vertex_buffer_data.push_back(v); //v1
	v.pos.Set(-0.5f * length, -0.5f * length, 0.f);	v.color = color; v.normal.Set(0, 0, 1);	v.texcoord.Set(0, 0); vertex_buffer_data.push_back(v); //v2
	v.pos.Set(0.5f * length, -0.5f * length, 0.f);	v.color = color; v.normal.Set(0, 0, 1);	v.texcoord.Set(500, 0); vertex_buffer_data.push_back(v); //v3
...
Mesh* MeshBuilder::GenerateSphere(const std::string& meshName, Color color, unsigned numStack, unsigned numSlice, float radius)
{
	...
	for (unsigned stack = 0; stack < numStack + 1; ++stack)
	{
		...
		for (unsigned slice = 0; slice < numSlice + 1; ++slice)
		{
			...
			//v.texcoord.Set((sphereZ(phi,theta)/abs(sphereX(phi,theta)) + 1)/2, (sphereY(phi, theta) / abs(sphereX(phi, theta)) + 1)/2);
...
Mesh* MeshBuilder::GenerateCylinder(const std::string& meshName, Color color, float numStack, float numSlice, float height, float radsize)
{
	int index = 0;
	Vertex vertex;
	std::vector<Vertex> vertex_buffer_data;
	...
	float heightofStack = height / numStack;
	...
	for (float stack = 0; stack < numStack; stack++)
	{
		for (float theta = 0; theta <= 360; theta += 10)
		{
			...
			vertex.pos.Set(radius * cos(theta_rad), -height / 2 + stack * heightofStack, radius * sin(theta_rad));
			vertex.color = color;
			vertex.normal = Vector3(cos(theta_rad), 0, sin(theta_rad));
			vertex_buffer_data.push_back(vertex);
			index_buffer_data.push_back(index++);

			vertex.pos.Set(radius * cos(theta_rad), -height / 2 + (stack + 1) * heightofStack, radius * sin(theta_rad));
			vertex.color = color;
			vertex.normal = Vector3(cos(theta_rad), 0, sin(theta_rad));
			vertex_buffer_data.push_back(vertex);
			index_buffer_data.push_back(index++);
...
Mesh* MeshBuilder::GenerateText(const std::string& meshName, unsigned numRow, unsigned numCol)
{
	...
	for (unsigned row = 0; row < numRow; ++row)
	{
		for (unsigned col = 0; col < numCol; ++col)
		{
			...
			v.pos.Set(.5,.5, 0.f);   v.normal.Set(0, 0, 1);	v.texcoord.Set(width * (col + 1), height*(numRow-row)); vertex_buffer_data.push_back(v); //v0 +,+
			v.pos.Set(-.5,.5, 0.f);  v.normal.Set(0, 0, 1);	v.texcoord.Set(width * (col), height*(numRow -row)); vertex_buffer_data.push_back(v); //v1 -,+
			v.pos.Set(-.5,-.5, 0.f); v.normal.Set(0, 0, 1);	v.texcoord.Set(width * col, height*(numRow - row - 1)); vertex_buffer_data.push_back(v); //v2 -,-
			v.pos.Set(.5, -.5, 0.f); v.normal.Set(0, 0, 1);	v.texcoord.Set(width * (col+1), height*(numRow - 1 - row)); vertex_buffer_data.push_back(v); //v3 +,-
