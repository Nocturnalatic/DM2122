...
void SceneModel::Init()
{
	...
	glUniform1i(m_parameters[U_NUMLIGHTS], 1); //Change for multiple lights
	// Light 1
	{
	...
	meshList[GEO_QUAD] = MeshBuilder::GenerateQuad("quad",Color(.7,.7,.7),1.f);
	...
	meshList[GEO_BLEND]->textureID = LoadTGA("Image//sidm.tga");
	...
	meshList[GEO_LIGHTBALL] = MeshBuilder::GenerateSphere("Light",Color(0,0,1), 10, 10, 1);
	meshList[GEO_LIGHTBALL_2] = MeshBuilder::GenerateSphere("Light2", Color(1, 0, 0), 10, 10, 1);
	meshList[GEO_MODEL_1] = MeshBuilder::GenerateOBJ("chair", "OBJ//chair.obj");
	meshList[GEO_MODEL_1]->textureID = LoadTGA("Image//chair.tga");
	meshList[GEO_MODEL_2] = MeshBuilder::GenerateOBJ("doorman", "OBJ//doorman.obj");
	meshList[GEO_MODEL_2]->textureID = LoadTGA("Image//doorman.tga");
	meshList[GEO_MODEL_3] = MeshBuilder::GenerateOBJ("bottle", "OBJ//winebottle.obj");
	meshList[GEO_MODEL_3]->textureID = LoadTGA("Image//winebottle.tga");
	meshList[GEO_MODEL_4] = MeshBuilder::GenerateOBJ("dart", "OBJ//dart.obj");
	meshList[GEO_MODEL_4]->textureID = LoadTGA("Image//dart.tga");
	meshList[GEO_MODEL_5] = MeshBuilder::GenerateOBJ("board", "OBJ//dartboard.obj");
	meshList[GEO_MODEL_5]->textureID = LoadTGA("Image//dartboard.tga");
...
void SceneModel::Render()
{
	...
	RenderMesh(meshList[GEO_MODEL_1], true); //Chair
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(0,-3,0);
	modelStack.Scale(1, 1, 1);
	RenderMesh(meshList[GEO_MODEL_2], true); //Doorman
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(-2, -3.2, 0);
	modelStack.Scale(.5, .5, .5);
	RenderMesh(meshList[GEO_MODEL_3], true); //Bottle
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(0, 0.5, 5);
	modelStack.Rotate(90,0,1,0);
	modelStack.Rotate(180, 0, 1, 0);
	modelStack.Scale(.2, .2, .2);
	RenderMesh(meshList[GEO_MODEL_4], true); //Dart
	modelStack.PopMatrix();
	...
	modelStack.Rotate(90, 0, 1, 0);
	modelStack.Scale(.3, .3, .3);
	RenderMesh(meshList[GEO_MODEL_5], true); //Board
	modelStack.PopMatrix();
